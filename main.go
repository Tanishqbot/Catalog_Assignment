...
func processTestCase(testCase string) *big.Int {
	...
	prodNums := make([]*big.Int, k) // using lagrange interpolation
	...
	for i := 0; i < k; i++ {
		...
		for j := 0; j < k; j++ {
			...
			termNum := new(big.Int).Neg(xj)  // (-x_j)
			prodNum.Mul(prodNum, termNum)    // Multiply into numerator

			termDen := new(big.Int).Sub(xi, xj)   // (x_i - x_j)
			prodDen.Mul(prodDen, termDen)     // Multiply into denominator
		}
	...
	for i := 0; i < k; i++ {
		...
		div := new(big.Int).Div(productDenominators, den)  // productDenominators / den
		term := new(big.Int).Mul(y, num)  // y_i * numerator
		term.Mul(term, div)  // Multiply by div


		sumNum.Add(sumNum, term) // adding to the total sum
	}

	c := new(big.Int).Div(sumNum, productDenominators)  // c = sumNum/ productDenominators
	return c
}